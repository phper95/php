...
class RoleModel extends CommonModel {
    ...
    function getGroupUserList($groupId, $fileds=array('id','nickname','email'))
    {	
    	$allow = array('id','nickname','account','last_login_time','last_login_ip', 'status', 'login_count', 'remark','email');
    	$fileds = array_intersect($allow, $fileds);
//        $table = $this->tablePrefix.'role_user';
//        $rs = $this->db->query('select b.'.implode(',b.', $fileds).' from '.$table.' as a ,'.$this->tablePrefix.'user as b where a.user_id=b.id and  a.role_id='.$groupId.' ');
        $rs = D('User')->where(array('role_id'=>$groupId))->field(implode(',', $fileds))->select();
