...
//	    		$this->_updateMemberNews($movie, $online_options);
    ...
    private function _updateMovieRelateWmean($movie, $tags, $zones, $showtimes){
    	...
    	// step5 遍历与该影片相关的所有影片，并且插入数据库中。 并且计算出相似度
     ...
     * 定时推送百度Push
     */
    public function doPushNotice(){
    	$_open_type = array(
    			'url' => 1,
				'app' => 3
    	);
    	$PushNotice = D('PushNotice');
    	$map = array('is_send'=>0,'send_time'=>array('elt',toDate(NOW_TIME))); // 找出待发送，并且发送时间小于当前时间的记录，一一发送
    	$list = $PushNotice->where($map)->select();
    	if (!empty($list)) {
//     		load('@.pushNotice');
    		load('@.pushNotice3');
    		$pushNotice = new Push_Notice3();
    		$message = array();
    		foreach ($list as $item) {
    			$bar_style = json_decode($item['bar_style'], true);
    			$after_click = json_decode($item['after_click'], true);
    			$open_type = $url = $user_confirm = '';
    			foreach ($after_click as $key=>$tmp) {
    				$open_type = $key;
    				$url = $tmp[0];
    				$user_confirm = $tmp[1];
    			}
    			...
    			$message['notification_basic_style'] = $bar_style['base'];
    			$message['open_type'] = $_open_type[$open_type];
    			...
    			$custom_content = json_decode($item['other_info'], true);
    			$custom_content['gm_opt'] = (int)$custom_content['gm_opt'];
...
//     					$rst = $pushNotice->pushMessage_ios($message, $user_id,'',1,2);
    				...
    				if (!empty($str)) {
    					$data = array('id'=>$item['id'], 'os' => implode(',', $os_str));
    					$PushNotice->save($data);
    					$this->error($str, 100);
    				} else {
    					$data = array('id'=>$item['id'], 'is_send'=>1);
    					$PushNotice->save($data);
    					$this->success('push_OK');
    				}
    			}
    		}
    	} else {
    		$this->success('push_ok_no_list');
    	}
    }
