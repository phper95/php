...
//    	$this->assign('adv_show_type',$rst);
    ...
    function _before_update(){
    	$arr = array('show_list','show_window','show_search','show_tag','show_qrcode','show_same','show_grapher','show_read','show_fullscreen');
    	foreach ($arr as $val) {
    		$avg = I($val);
    		...
    		foreach ($ppc_list as $ppc) {
    			...
    			if (empty($list)) { // 不存在则插入
    				$data = array(
    					...
    					$online_advs => $adv['id'],
    					$online_times => date('Y-m-d H:i:s', NOW_TIME)
    				);
    				...
    				$i = 0;
    				$tmp = '';
    				foreach ($online_adv_time as $key=>$val) {
    					if ($i==$position) {
    						$tmp = $val;
    						if ($i == 0) { // 如果是在第一位，则用当前时间
    							$online_adv_time[$key] = date('Y-m-d H:i:s', NOW_TIME);
    						} else {
    							$online_adv_time[$key] = $val;
    							$tmp = date('Y-m-d H:i:s',(strtotime($val)-1)); // 比这个小1秒
    						}
    						$i++;continue;
    					}
    					if ($tmp != '') { // 找到位置了 time 顺利插入
    						$online_adv_time[$key] = $tmp;
    						$tmp = $val;
    					}
    					$i++;
    				}
    				...
    				if (empty($online_adv)) { // 如果为空的话。 则添加进去
    					$online_adv = array($adv['id']);
    					$online_adv_time = array(date('Y-m-d H:i:s', NOW_TIME));
    				} else if ($i<=$position) { // 如果$position 大于最后一个位置 则添加到最后
    					$online_adv[] = $adv['id'];
    					$time = end($online_adv_time);
    					$online_adv_time[] = date('Y-m-d H:i:s',(strtotime($time)-1)); // 比这个小1秒
    				}
