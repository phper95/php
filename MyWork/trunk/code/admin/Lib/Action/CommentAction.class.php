...
//        $map['account'] = array('like',"%".$_POST['name']."%");
    ...
    public function topicList() {
    	...
    	$this->_getList('CommentTopic', $alert);
    }
    
    public function advList() {
    	$alert = I('get.alert',false);
    	$this->_getList('CommentAdv', $alert);
    }
    
    public function movieList() {
    	$alert = I('get.alert',false);
    	$this->_getList('CommentMovie', $alert);
    }
    
    public function WeiList() {
    	$alert = I('get.alert',false);
    	$this->_getList('CommentWei', $alert);
    }
    ...
    public function showInfo(){
    	...
    	if ($type == 'movie') { // 展示影片的评论
    		...
    		$this->assign('members', $nameArr);
    		$this->assign('obj',$adv);
    	} elseif ($type == 'topic') { // 专辑的评论
    		$topic = D('Topic')->where(array('id'=>$id))->field('id,name')->find();
    		$model = D('CommentTopic');
    		...
    		$this->assign('members', $nameArr);
    		$this->assign('obj',$topic);
    	} elseif ($type == 'wei') { // 专辑的评论
    		$wei = D('Wei')->where(array('id'=>$id))->field('id,title as name')->find();
    		$model = D('CommentWei');
    		$map = array('wei_id'=>$id);
    		if (!empty($member_id)){
    			$map['user_id'] = $member_id;
...
//    				$this->display('index'); 
	...
	public function heXie() {
    	...
    	$rst = array('rst'=>0, 'msg'=>'');
    	$model_name = array(
    		'movie'=>array('comment'=>'CommentMovie','bk'=>'CommentMovieBk'), 
    		'adv'=>array('comment'=>'CommentAdv','bk'=>'CommentAdvBk')
    	);
    	if (empty($type) || empty($id) || empty($reason) || !isset($model_name[$type]) || !isset($this->_msg[$reason])) {
    		$rst['msg'] = '参数错误';
    	} else {
    		$tmp = $model_name[$type];
    		$comment = D($tmp['comment']);
    		$comment_bk = D($tmp['bk']);
    		$content = $this->_msg[$reason];
    		$rst['data']['ids'] = array();
    		if ($hexie == 'all') {
    			$map = array('id'=>$id);
    			$one = $comment->where($map)->field('user_id,movie_id')->find();
    			$map = array('user_id'=>$one['user_id'], 'movie_id'=>$one['movie_id']);
    			$ids = $comment->where($map)->getField('id', true);
    			$error = false;
    			foreach ($ids as $tmp_id) {
    				$msg = $this->_updateContent($comment, $comment_bk, $tmp_id, $content);
    				if (is_string($msg)){
    			...
    			$rst['rst'] = $error ? 0 : 1;
    		} else if ($hexie == 'theall'){ // 屏蔽该用户所有评论
    			$map = array('id'=>$id);
    			$one = $comment->where($map)->field('user_id')->find();
	    		...
	    		$msg = $this->_updateContent($comment, $comment_bk, $id, $content);
	    		if (is_string($msg)) {
	    			$rst['msg'] = $msg;
	    		} else {
	    			if ($msg === false) {
     ...
     * @param unknown_type $comment_bk
     * @param unknown_type $id
     * @param unknown_type $content
     */
    public function _updateContent($comment, $comment_bk, $id, $content) {
    	$map = array('comment_id'=>$id);
    	$one = $comment_bk->where ($map)->find();
    	if (!empty($one)) {return false;}
		$map = array ('id' => $id );
		$tmp_content = $comment->where ( $map )->getField ( 'comment_content' );
		$data = array ('comment_id' => $id, 'comment_content' => $tmp_content );
		$comment_bk->create ( $data );
		if (false === $comment_bk->add ()) {
			return '备份信息出错';
		} else {
			$data = array ('id' => $id, 'comment_content' => $content );
			if (false === $comment->save ( $data )) {
				return '修改内容出错';
			} else {
				return true;
			}
